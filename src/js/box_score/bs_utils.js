...
function formatSummary(summary) {
    const vColor = getLogoColor(summary.htlg)
    const aColor = getLogoColor(summary.atlg)
    $('#away_team_logo')
        .text(summary.atlg)
    ...
    $('.summary-box-score tbody tr:nth-child(2) th')
        .eq(0)
        .text(summary.ata)
    $('.summary-box-score tbody tr:nth-child(3) th')
        .eq(0)
        .text(summary.hta)
    ...
    if (summary.rm) {
        ...
        var away = $('#away_team_pts')
            .text(summary.atpts)
            .removeClass(COLOR.GREEN)
        var home = $('#home_team_pts')
            .text(summary.htpts)
            .removeClass(COLOR.GREEN)
...
function formatTeamStatsData(teamStats) {
    return {
        1: ['', '', teamStats.fgm.toString() + '-' + teamStats.fga.toString(), toPercentage(teamStats.fgm / teamStats.fga), teamStats.tpm.toString() + '-' + teamStats.tpa.toString(), toPercentage(teamStats.tpm / teamStats.tpa), teamStats.ftm.toString() + '-' + teamStats.fta.toString(), toPercentage(teamStats.ftm / teamStats.fta), teamStats.oreb.toString(), teamStats.dreb.toString(), teamStats.reb.toString(), teamStats.ast.toString(), teamStats.stl.toString(), teamStats.blk.toString(), teamStats.tov.toString(), teamStats.pf.toString(), '', teamStats.s.toString()],
        2: [
            '',
            teamStats.fbpts,
            teamStats.bpts,
...
function highlightSummaryTable() {
    for (let i = 0; i < 16; i++) {
        // [0...15]
        var vpts = parseInt(
            $('.summary-box-score tbody tr:nth-child(2) td')
                .eq(i)
                .text()
        )
        var hpts = parseInt(
            $('.summary-box-score tbody tr:nth-child(3) td')
                .eq(i)
                .text()
        )
        $('.summary-box-score tbody tr:nth-child(2) td')
...
function highlightPlayerRowHelper(index, el) {
    ...
    if (parseInt(result[2].text()) >= 5) result[2].addClass(COLOR.GREEN)
    ...
    if (parseInt(result[3].text()) >= 5) result[3].addClass(COLOR.GREEN)

    if (parseInt(result[4].text()) === 0) result[4].addClass(COLOR.GREEN)
    else if (parseInt(result[4].text()) >= 5) result[4].addClass(COLOR.RED)
    ...
    doubles += ')'
    if (parseInt($(children[15]).text()) === 6) $(children[15]).addClass(COLOR.RED)

    if (count === 3) {
        // tri-db
        $(el)
            .addClass(BG_COLOR.BG_ORANGE)
            .attr('title', 'Triple Double! ' + doubles)
    } else if (count === 4) {
        $(el)
            .addClass(BG_COLOR.BG_PURPLE)
            .attr('title', 'Quadruple Double! ' + doubles)
    } else if (count === 2) {
        //db-db
        $(el)
            .addClass(BG_COLOR.BG_BLUE)
            .attr('title', 'Double Double! ' + doubles)
    } else if (count === 5) {
        $(el)
            .addClass(BG_COLOR.BG_GREEN)
            .attr('title', 'Quintuple Double! ' + doubles)
    }
}
